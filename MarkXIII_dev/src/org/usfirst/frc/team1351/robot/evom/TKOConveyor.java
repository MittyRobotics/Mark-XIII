		...
		try
		{
			while (conveyorThread.isThreadRunning())
			{
				...
				if (testEnabled)
				{
					...
					{
						TKOHardware.getConveyorTalon(2).set(0.0); //TODO This line is going to break it, commented out for now
					}
					// TODO fix later
					/*
					 * if (TKOHardware.getJoystick(2).getRawButton(4) && TKOHar dware.getSwitch(0).get()) {
					 * TKOHardware.getConveyorTalon(2).set(-0.4); timeout = System.currentTimeMillis(); } else if
					 * (TKOHardware.getJoystick(2).getRawButton(4) && (System.currentTimeMillis() - timeout <= 200)) { System.out.println(
					 * "Running conveyor backward"); TKOHardware.getConveyorTalon(2).set(0.4); } else if
					 * (TKOHardware.getJoystick(2).getRawButton(5)) { TKOHardware.getConveyorTalon(2).set(0.4); } else {
	...
	public void getBall()
	{
		try
		{
			if (TKOHardware.getJoystick(3).getRawButton(4))
			{
				...
				{
					TKOHardware.getConveyorTalon(0).set(0.2);
					// test values
				}
				else if (!TKOHardware.getSwitch(0).get())
				{
					TKOHardware.getConveyorTalon(0).set(-0.4);
				}
			}
		}
		catch (TKOException e)
